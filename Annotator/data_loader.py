...
def convert_examples_to_features(args, examples, tokenizer, max_length):
    ...
    label_list_len = len(processor.get_labels()) #['ambiguous', 'negative', 'neutral', 'positive'] 4
    ...
    #    if example.text_b is not None: #all is none
    ...
    #    print(k) #input_ids, token_type_ids, attention_mask
    features = []
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    cached_features_file = os.path.join( #'data/group/cached_goemotions_bert-base-cased_50_train'
        args.data_dir,
...
#        features = torch.jit.load(cached_features_file)
#    else:
    logger.info("Creating features from dataset file at %s", args.data_dir)
    if mode == "train":
        examples = processor.get_examples("train") #list, len=43410, 
    elif mode == "dev":
